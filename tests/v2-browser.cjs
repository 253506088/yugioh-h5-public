...
const out=path.join(root,'output/v2-browser','run-'+new Date().toISOString().replace(/[:.]/g,'-'));
...
(async()=>{
  ...
  await context.addInitScript(()=>{
    localStorage.setItem('duel-sanctuary-welcomed-v2','true');localStorage.setItem('duel-sanctuary-online-art-v2','false');
  ...
  await check('single HTML opens offline with all expansion modules',async()=>{
    ...
    assert.deepEqual(info,{version:2,cards:176,decks:10,overflow:false});await screenshot('01-desktop-field');assert.deepEqual(report.errors,[]);
  });
  await check('ten presets are selectable with an independently selected opponent',async()=>{
    await page.click('[data-action="new-game"]');assert.equal(await page.locator('.deck-roster-item').count(),10);
    await page.click('[data-action="choose-deck"][data-deck="crystron"]');await page.selectOption('#opponent-deck','cyber');await screenshot('02-ten-deck-selector');
    ...
    for(const deck of ['blue','dark','hero','blackwing','junk','utopia','qliphort','exodia','cyber','crystron']){
  ...
  await check('archive filters, pendulum descriptions, search and navigation',async()=>{
    await page.click('[data-action="library"]');assert.equal(await page.locator('#library-grid .library-card').count(),176);
  ...
  async function stage(deck,entries,options={}){
    return page.evaluate(({deck,entries,options})=>{
      ...
      for(const [owner,zone,id,props={}] of entries){
        ...
        else if(['extraMonster','fieldSpell'].includes(zone))e.state.players[owner][zone]=card;else e.state.players[owner][zone].push(card);
  ...
  await fs.mkdir(out,{recursive:true});await fs.writeFile(path.join(out,'report.json'),JSON.stringify(report,null,2));await fs.writeFile(path.join(root,'output/v2-browser-report.json'),JSON.stringify(report,null,2));
