 ...
 * Dedicated GX tests separately verify costs, summon procedures and outcomes. */
...
const report={cards:[],errors:[]},out=path.resolve(__dirname,'../output/gx-sweep');fs.mkdirSync(out,{recursive:true});
const cards=D.CARD_LIST.filter(c=>c.early&&c.releaseYear>=2006&&c.releaseYear<=2008);
