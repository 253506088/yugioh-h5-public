...
(function(root){
 ...
 const original=Object.values(E.defs).filter(a=>!a.trigger&&!a.cardActivation&&!a.inherent&&!a.supersededByOriginal&&!a.mode.startsWith('copied:')&&a.zones.includes('monsters')&&!hosts.has(a.id)&&D.CARDS[a.id]?.releaseYear<=2012);
 ...
 for(const c of D.CARD_LIST.filter(c=>c.releaseYear>=2009&&c.releaseYear<=2012&&!hosts.has(c.id))){const notes=notesById.get(c.id);if(notes?.size)c.implementationNote=[...notes].join(' ');}
