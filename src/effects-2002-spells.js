...
(function(root){
 ...
 // ---- Book of Moon / Taiyou -----------------------------------------------------------
 S('Book of Moon',{condition:(e,c)=>allM(e).some(m=>m.faceUp&&!['link','token'].includes(CARDS[m.id].type)),inputs:target('选择变为里侧守备的表侧怪兽',e=>allM(e).filter(m=>m.faceUp&&!['link','token'].includes(CARDS[m.id].type)),'destroy'),resolve:(e,c)=>e.setPosition(first(c),'defense',c.source,true),aiResponse:(e,c,w)=>w.attack&&w.attack.stage==='declare'?1200:0});
 ...
 S('Wave-Motion Cannon',{resolve:()=>{},aiScore:300});A('Wave-Motion Cannon','fire',{zones:['spells'],requiresField:true,condition:(e,c)=>(H.source(e,c)?.card?.waveCounters||0)>0,resolve:(e,c)=>{const m=H.source(e,c)?.card;if(!m)return;const n=m.waveCounters||0;delete m.waveCounters;if(e.find(c.uid)&&n>0){e.move(c.uid,'grave',{kind:'effect-send',source:c.source,byOwner:c.owner});burn(e,c,n*1000);}},aiScore:(e,c)=>((H.source(e,c)?.card?.waveCounters||0)*1000>=e.state.players[1-c.owner].lp)?5000:0});
