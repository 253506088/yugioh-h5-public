...
(function(root){
 ...
 cast('Soul Charge',(e,c)=>grave(e,c.owner,m=>monster(m)&&e.canSpecial(c.owner,m,{via:'revive'})),(e,c)=>{let n=0;for(const u of args(c))if(revive(e,c,u))n++;if(n){e.state.players[c.owner].lp-=n*1000;e.log('effect','灵魂补充失去 '+(n*1000)+' LP',c.owner,{amount:n*1000,cardId:c.sourceId});e.checkWin();}},{min:1,max:5,summons:true,once:H.once('soul-charge','name'),condition:(e,c)=>e.freeMain(c.owner)>0&&!(e.state.players[c.owner].eraBattleConducted===e.state.turn),cost:(e,c)=>{e.state.players[c.owner].skipBattleTurn=e.state.turn;},role:'special'});
