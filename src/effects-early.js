(function(root){
 ...
 E.on('move',(e,v)=>{for(const def of moves.get(v.id)||[])if(def.test(e,v))e.addTrigger(v.uid,v.id+'::'+def.mode,v,{owner:e.find(v.uid)?.owner??v.owner,mandatory:def.mandatory});if(v.to==='grave'){const m=e.find(v.uid)?.card;if(m)m.earlySent={turn:e.state.turn,from:v.from,kind:v.kind,byOwner:v.byOwner,byEffect:v.byEffect,previous:v.previous};}});
