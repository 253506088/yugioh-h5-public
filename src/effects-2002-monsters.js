...
(function(root){
 ...
 C('Lava Golem').specialOnly='lava-golem';mark('Lava Golem','解放对方2只怪兽在其场上特殊召唤；召唤回合召唤者不能通常召唤／盖放；当前控制者在自己的准备阶段受1000效果伤害');
 A('Lava Golem','special',{zones:['hand'],inherent:true,summons:true,
  condition:(e,c)=>{const pool=foeM(e,c).filter(m=>e.canTribute(m,1-c.owner)),m=e.find(c.uid)?.card;return pool.length>=2&&!e.state.normalUsed&&m&&e.canSpecial(1-c.owner,m,{via:'lava-golem',summoner:c.owner})&&e.freeZones(1-c.owner,m,{materials:pool.map(m=>m.uid)}).length>0;},
  inputs:(e,c)=>[g(e,c,'target','选择解放的2只对方怪兽',foeM(e,c).filter(m=>e.canTribute(m,1-c.owner)),2,2,'cost')],
  cost:(e,c)=>{if(!e.freeZones(1-c.owner,e.find(c.uid).card,{materials:args(c)}).length)throw new root.DuelRuleError('解放后没有可用的怪兽区域。');tribute(e,c,args(c),1-c.owner);},
  resolve:(e,c)=>{const m=e.special(1-c.owner,c.uid,{via:'lava-golem',summoner:c.owner});if(m)e.state.players[c.owner].normalSummonLockedTurn=e.state.turn;},
  aiScore:(e,c)=>foeM(e,c).length>=2?800:-100});
 onStandby('Lava Golem',{resolve:(e,c)=>e.damage(c.owner,1000,'效果',c.source)},{optional:false});
