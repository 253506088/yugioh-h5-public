...
(function(root){
 ...
 extend('attackValue',function(prior,m,...rest){const n=prior.call(this,m,...rest);if(!is(m,'Giganticastle'))return n;const ids=this.find(m.uid)?.card.eraSynchroMaterials||[];return n+200*ids.filter(id=>CARDS[id]?.type!=='tuner'&&!this.isTuner({id})).length;});
 ...
 extend('canTarget',function(prior,m,source){const f=this.find(m.uid);if(!f)return prior.call(this,m,source);if(this.state.players[f.owner].eraAzureUntil>=this.state.turn&&this.race(m)==='龙族'&&source&&source.owner!==f.owner)return false;return prior.call(this,m,source);});
 ...
 extend('attackValue',function(prior,m,...rest){const n=prior.call(this,m,...rest);return is(m,'Downerd Magician')?n+200*overlayCount(this,m.uid):n;});
 passive('Downerd Magician',{piercing:(e,s,m)=>m.uid===s.card.uid});
