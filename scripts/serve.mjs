...
const server = createServer(async (request, response) => {
  try {
    ...
    // Development previews may share a checkout with a live PVP database.
    if (/(^|[\\/])\./.test(pathname) || /\.(?:sqlite|db)(?:$|[.\-:\\/])/i.test(pathname)) { response.writeHead(404); response.end('Not found'); return; }
...
server.listen(port, '127.0.0.1', () => console.log(`Duel Sanctuary: http://127.0.0.1:${server.address().port}`));
