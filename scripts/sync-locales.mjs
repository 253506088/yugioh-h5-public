...
const missingPendulum=cards.filter(c=>c.type==='pendulum').flatMap(c=>['zh-CN','en','ja'].filter(l=>!output[c.id]?.locales[l]?.pendulumDescription).map(language=>({id:c.id,language})));
